...
void TimeManagement::init(Search::LimitsType& limits, Color us, int ply) {
    ...
    TimePoint minimum_time = std::min(
        static_cast<TimePoint>(OptionValue::MinimumThinkingTime),
        limits.time[us] / 2
    );
    ...
    timeLeft = std::max(timeLeft, minimum_time);
    ...
    maximumTime = TimePoint(std::min(
        1.0 * limits.time[us] - moveOverhead,
        std::max(
            max_scale * optimumTime,
            static_cast<double>(minimum_time)
        )
    ));
